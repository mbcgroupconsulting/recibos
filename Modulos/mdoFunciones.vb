...
Reintentar:
    ...
    Public Function nConsultaContpaq(ByRef ds As DataSet, ByVal SQL As String, Optional ByVal Tabla As String = "Table", Optional ByVal Limpiar As Boolean = True) As DataRow()
        ' ValidaConexion()
        nConsultaContpaq = Nothing
        Try
Reintentar:
            Dim Comando As SqlDataAdapter
            Dim aSQL As String(), i As Integer
            ...
            For i = 0 To aSQL.GetUpperBound(0)
                Comando = New SqlDataAdapter(aSQL(i), CONTPAQCONEXION)
                Comando.Fill(ds, Tabla)
            Next
            nConsultaContpaq = Nothing
            If aSQL.GetUpperBound(0) = 0 Then
                If ds.Tables(Tabla) Is Nothing = False Then
                    nConsultaContpaq = ds.Tables(Tabla).Select("TRUE")
                End If
                If nConsultaContpaq.Length = 0 Then nConsultaContpaq = Nothing
            Else
                nConsultaContpaq = Nothing
            End If

        Catch ex As Exception
            If CONTPAQCONEXION.State <> ConnectionState.Open Then
                ValidaConexion()
                GoTo Reintentar
            Else
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
...
Reintentar:
    ...
    Public Function nExecuteContpaq(ByVal SQL As String, Optional ByVal Tabla As String = "", Optional ByVal sId As String = "") As Boolean
        Dim Sentencia As String
        Try
Reintentar:
            Dim comando As New SqlClient.SqlCommand
            ...
            Sentencia = SQL
            comando.Connection = CONTPAQCONEXION

            comando.CommandType = CommandType.Text
            comando.Transaction = TransaccionContpaq

            comando.CommandText = SQL
            comando.ExecuteNonQuery()

            nExecuteContpaq = True
            'If Servidor.Central = False And InStr(Sentencia, "INTO Bitacora ") = 0 And Tabla <> "" Then
            '    Try
            '        Dim IdSQL As String, Id As Long, Accion As Integer
    ...
    Public Function nExecuteKiosko(ByVal SQL As String, Optional ByVal Tabla As String = "", Optional ByVal sId As String = "") As Boolean
        Dim Sentencia As String
        Try
Reintentar:
            Dim comando As New SqlClient.SqlCommand
            ...
            Sentencia = SQL
            comando.Connection = KIOSKOCONEXION
...
Reintentar:
    ...
    Public Function ExecuteKiosko(ByVal SQL As String, ByRef IdGenerate As String, Optional ByVal Tabla As String = "", Optional ByVal sId As String = "") As Boolean
        Dim Sentencia As String
        Dim Aux As String
        Try
Reintentar:
            ...
            If mdoObjetos3.Central = False And InStr(Sentencia, "INTO Bitacora ") = 0 And Tabla <> "" Then
                Try
                    Dim IdSQL As String, Id As Long, Accion As Integer
                    If sId = "" Then sId = IdGenerate

                    SQL = "SELECT MAX(RIGHT(IdSQL,10)) FROM Bitacora WHERE LEFT(IdSQL,2)='" & Strings.Right(mdoObjetos3.IdSeccion, 2) & "'"
                    Id = Val(ObtencampoDRKiosko(SQL, 0)) + 1
                    IdSQL = Strings.Right(mdoObjetos3.IdSeccion, 2) & Format(Id, StrDup(10, "0"))
                    ...
                    If InStr(Sentencia, "GETDATE()") > 0 Then
                        mdoObjetos3.Actualizar()
                        Sentencia = Replace(Sentencia, "GETDATE()", "CAST('" & mdoObjetos3.Fecha_Hora & "' AS smalldatetime)", , , CompareMethod.Text)
                    End If
...
Revalidar:
    ...
    Public Sub ValidaConexionContpaq()
        Dim Conectado As Boolean = False
Revalidar:
        If CONTPAQCONEXION.State = ConnectionState.Closed Then

            Do
                bValidandoContpaq = True
                Try
                    If mdoObjetos2.Central Then
                        'CONTPAQCONEXION.ConnectionString = "Server=localhost;uid=sa;Password=1123581321;DataBase=" & mdoObjetos2.sBase
                        CONTPAQCONEXION.ConnectionString = "Server=192.168.1.224\compac;uid=sa;Password=12345;DataBase=" & mdoObjetos2.sBase
                    Else
                        'CONTPAQCONEXION.ConnectionString = "Server=localhost;uid=sa;Password=1123581321;DataBase=" & mdoObjetos2.sBase
                        CONTPAQCONEXION.ConnectionString = "Server=192.168.1.224\compac;uid=sa;Password=12345;DataBase=" & mdoObjetos2.sBase
                    End If
                ...
                Catch ex As Exception
                    ...
                    End If
                End Try
            Loop While CONTPAQCONEXION.State = ConnectionState.Closed Or Not Conectado
            bValidando = False
            MessageBox.Show("El sistema se ha logrado reconectar", "", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Threading.Thread.Sleep(2000)
        Else
    ...
    Public Sub ValidaConexionKiosko()
        Dim Conectado As Boolean = False
Revalidar:
        If KIOSKOCONEXION.State = ConnectionState.Closed Then

            Do
                bValidandoContpaq = True
                Try
                    If mdoObjetos2.Central Then
                        'KIOSKOCONEXION.ConnectionString = "Server=localhost;uid=sa;Password=1123581321;DataBase=" & mdoObjetos3.sBase
                        KIOSKOCONEXION.ConnectionString = "Server=192.168.1.222;uid=sa;Password=kiosko2016;DataBase=" & mdoObjetos3.sBase
                    Else
                        'KIOSKOCONEXION.ConnectionString = "Server=localhost;uid=sa;Password=1123581321;DataBase=" & mdoObjetos3.sBase
                        KIOSKOCONEXION.ConnectionString = "Server=192.168.1.222;uid=sa;Password=kiosko2016;DataBase=" & mdoObjetos3.sBase
                    End If
                ...
                Catch ex As Exception
                    ...
                    End If
                End Try
            Loop While KIOSKOCONEXION.State = ConnectionState.Closed Or Not Conectado
            bValidando = False
            MessageBox.Show("El sistema se ha logrado reconectar", "", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Threading.Thread.Sleep(2000)
        Else
...
reintentar:
    ...
    Public Function nConsultaContpaq(ByVal SQL As String) As DataRow()
...
reintentar:
            ...
            'Next
            Comando = New SqlDataAdapter(SQL, CONTPAQCONEXION)
            Comando.Fill(ds, Tabla)

            nConsultaContpaq = Nothing
            'If aSQL.GetUpperBound(0) = 0 Then
            If ds.Tables(0) Is Nothing = False Then
                nConsultaContpaq = ds.Tables(0).Select("true")
            End If
            If nConsultaContpaq.Length = 0 Then nConsultaContpaq = Nothing
            'Else
            '    nConsulta = Nothing
    ...
    Public Function nConsultaKiosko(ByVal SQL As String) As DataRow()



        Dim ds As DataSet
        Dim Tabla As String = "Table", Limpiar As Boolean = True
        nConsultaKiosko = Nothing
        Try
reintentar:
            Dim Comando As SqlDataAdapter
            ...
            'Next
            Comando = New SqlDataAdapter(SQL, KIOSKOCONEXION)
            Comando.Fill(ds, Tabla)

            nConsultaKiosko = Nothing
            'If aSQL.GetUpperBound(0) = 0 Then
            If ds.Tables(0) Is Nothing = False Then
                nConsultaKiosko = ds.Tables(0).Select("true")
            End If
            If nConsultaKiosko.Length = 0 Then nConsultaKiosko = Nothing
            'Else
            '    nConsulta = Nothing
            'End If

        Catch ex As Exception
            If KIOSKOCONEXION.State <> ConnectionState.Open Then
                ValidaConexionKiosko()
...
Reintentar:
    ...
    Public Function ObtencampoDRKiosko(ByVal SQL As String, ByVal Indice As Integer) As String
        Try
Reintentar:
            Dim oComando As New SqlCommand(SQL, KIOSKOCONEXION)
            oComando.Transaction = Transaccion
            Dim drDato As SqlDataReader = oComando.ExecuteReader
            If drDato.Read Then
                ObtencampoDRKiosko = drDato.Item(Indice).ToString
            Else
                ObtencampoDRKiosko = ""
            End If
            drDato.Close()
