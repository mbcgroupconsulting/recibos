...
Public Class ImportarSueldosConciliacion
        ...
        Try
            If lsvLista.CheckedItems.Count > 0 Then
                ...
                For Each producto As ListViewItem In lsvLista.CheckedItems
                    'buscamos que no este ya conciliado esta parte
                    SQL = "select * from conciliacion where iIdConciliacion=" & Trim(producto.SubItems(1).Text)

                    Dim rwConciliacion As DataRow() = nConsulta(SQL)

                    If rwConciliacion Is Nothing = False Then
                        If rwConciliacion(0)("iEstatus2") <> "2" Then
                            SQL = "update conciliacion set  fkIdUsuario2=" & idUsuario
                            SQL &= ",cUsuario2='" & nombresistema
                            SQL &= "',cDatosFactura='" & producto.SubItems(9).Text
                            ...
                            SQL &= "',iEstatus2=2"
                            SQL &= " where iIdConciliacion=" & producto.SubItems(1).Text

                            If nExecute(SQL) = False Then
                                MessageBox.Show("Error en el registro con los siguiente datos: iIdConciliacion:" & Trim(producto.SubItems(1).Text) & " Cargo:" & Trim(producto.SubItems(6).Text) & " Abono:" & Trim(producto.SubItems(7).Text) & ". El proceso concluira en ese registro. ", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                                Exit Sub
                            End If
                        End If
                    End If
