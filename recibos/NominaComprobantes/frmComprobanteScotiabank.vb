...
Public Class frmComprobanteScotiabank
        ...
        Try
            If File.Exists(lblRuta.Text) Then
                ...
                If book.Worksheets.Count >= 1 Then
                    ...
                    Dim Filas As Long = sheet.RowsUsed().Count() + 10
            ...
            If lsvLista.CheckedItems.Count > 0 Then
                ...
                For Each producto As ListViewItem In lsvLista.CheckedItems
                    ...
                    If importe > 0 Then
                        ...
                        dsReporte.Tables("Tabla").Columns.Add("Fecha2")
                        dsReporte.Tables("Tabla").Columns.Add("FechaLarga")
                        ...
                        fila.Item("Fecha2") = Format(Date.Parse(dtpFecha.Value), "yyyy/MM/dd")
                        ...
                        fila.Item("Hora") = Trim(dtpHora.Value.ToString.ToUpper.Substring(11, 8)) '+ " " + Trim(dtpHora.Value.ToString.ToUpper.Substring(20))
                        fila.Item("Cliente") = Trim(txtContrato.Text)
                        fila.Item("Cuenta") = Trim(txtCuenta.Text)
                        fila.Item("NombrePago") = producto.SubItems(CInt(NudBanco.Value)).Text
                        ...
                        fila.Item("MotivoPago") = AsignarBanco(producto.SubItems(CInt(NudBanco.Value)).Text)
                        ...
                        Dim oReporte As New ScotiabankNomina
                        oReporte.SetDataSource(dsReporte)
                        oReporte.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, direccioncarpeta & "\" & producto.SubItems(CInt(NudNombre.Value)).Text.ToUpper & " " & producto.SubItems(CInt(NudImporte.Value)).Text.ToUpper & " " & CDate(dtpFecha.Value).Day.ToString("00") & "-" & CDate(dtpFecha.Value).Month.ToString("00") & "-" & CDate(dtpFecha.Value).Year.ToString() & ".pdf")
    ...
    Private Sub txtContrato_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtContrato.TextChanged
    ...
    Function AsignarBanco(ByRef codigobanco As String) As String
        Try
            Dim banconame As String = ""
            SQL = "select* from bancos where clave = '" & codigobanco & "'"
            Dim rwFilas As DataRow() = nConsulta(SQL)

            If rwFilas Is Nothing = False Then
                Dim Fila As DataRow = rwFilas(0)
                banconame = Fila.Item("cBanco")
            End If
