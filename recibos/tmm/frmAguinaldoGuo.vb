Public Class frmAguinaldoGuo
        ...
        If lsvLista.CheckedItems.Count > 0 Then
            ...
            For Each producto As ListViewItem In lsvLista.CheckedItems
                ...
                If (Trim(producto.SubItems(0).Text) = "A") Then
                    ...
                    percepciones = Val(IIf(Trim(producto.SubItems(8).Text) = "", "0", Trim(producto.SubItems(8).Text)))
                    deducciones = Val(IIf(Trim(producto.SubItems(9).Text) = "", "0", Trim(producto.SubItems(9).Text)))
                    neto = Val(IIf(Trim(producto.SubItems(10).Text) = "", "0", Trim(producto.SubItems(10).Text)))

                    Dim fila As DataRow = fiscalvia.dsReporte.Tables("Tabla").NewRow
                    fila.Item("numtrabajador") = Trim(producto.SubItems(2).Text)
                    fila.Item("nombre") = Trim(producto.SubItems(3).Text)
                    fila.Item("rfc") = Trim(producto.SubItems(20).Text)
                    fila.Item("curp") = Trim(producto.SubItems(21).Text)
                    fila.Item("imss") = Trim(producto.SubItems(19).Text)
                    fila.Item("fecingreso") = Trim(producto.SubItems(6).Text).Substring(0, 10)
                    fila.Item("sucursal") = "LEON"
                    fila.Item("departamento") = "OPERACION"
                    fila.Item("puesto") = Trim(producto.SubItems(4).Text)
                    fila.Item("deldia") = txtperiodo.Text
                    fila.Item("aldia") = " "
                    fila.Item("sdodiario") = Trim(producto.SubItems(16).Text)
                    fila.Item("sdointegrado") = Trim(producto.SubItems(17).Text)
                    fila.Item("percepciones") = IIf(Trim(producto.SubItems(8).Text) = "", "0", Trim(producto.SubItems(8).Text))
                    fila.Item("deducciones") = IIf(Trim(producto.SubItems(9).Text) = "", "0", Trim(producto.SubItems(9).Text))
                    fila.Item("neto") = IIf(Trim(producto.SubItems(10).Text) = "", "0", Trim(producto.SubItems(10).Text))
                    ...
                    For x = 27 To 34
                        ...
                        If (Trim(lsvLista.Columns(x).Text) = "30") Then
                            If (Trim(producto.SubItems(x).Text) <> "") Then
                                Dim fila2 As DataRow = fiscalvia.dsReporte.Tables("Percepciones").NewRow
                                fila2.Item("numtrabajador") = Trim(producto.SubItems(2).Text)
    ...
    Private Sub tsbProcesar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbProcesar.Click
        ...
        Try
            ...
            Dim anchos As String() = "40,20,50,100,20,20,20,40,65,250,150,130,130,130,130,130,130,130,130,130,130,130,130".Split(",")

            Periodo = txtperiodo.Text 'tbRegistros.Rows(1).Item(8).ToString().Substring(0, 36)
