Public Class frmImportarGastos
        ...
        Try
            If lsvLista.CheckedItems.Count > 0 Then
                ...
                For Each producto As ListViewItem In lsvLista.CheckedItems
                    If Trim(producto.SubItems(4).Text) = "" Or Trim(producto.SubItems(6).Text) = "" Or Trim(producto.SubItems(7).Text) = "" Then
                        MessageBox.Show("Error en el registro con los siguiente datos: fecha expedición:" & Trim(producto.SubItems(2).Text) & " Proveedor:" & Trim(producto.SubItems(3).Text) & " Empresa:" & Trim(producto.SubItems(5).Text) & ". El proceso concluira en ese registro. ", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                        pnlProgreso.Visible = False
                        Exit Sub
                    Else
                        'Insertar nuevo
                        SQL = "EXEC setGastosInsertar 0,'" & Trim(producto.SubItems(1).Text)
                        SQL &= "','" & Format(Date.Parse(Mid((Trim(producto.SubItems(2).Text)), 1, 10)), "yyyy/dd/MM")
                        SQL &= "','" & Trim(producto.SubItems(3).Text)
                        SQL &= "'," & Trim(producto.SubItems(4).Text)
                        SQL &= "," & ((IIf(Trim(producto.SubItems(6).Text) = "", 0, Trim(producto.SubItems(6).Text))).ToString.Replace(",", "")).Replace(" ", "")
                        SQL &= ",'" & Format(Date.Parse(Mid((Trim(producto.SubItems(7).Text)), 1, 10)), "yyyy/dd/MM")
                        SQL &= "','" & Date.Now.ToShortDateString() & "'"
                        SQL &= ",'" & nombresistema & "','" & nombresistema
                        SQL &= "',1," & Trim(producto.SubItems(8).Text)
    ...
    Private Sub tsbProcesar_Click(sender As Object, e As EventArgs) Handles tsbProcesar.Click
        ...
        Try
            ...
            If tbRegistros Is Nothing = False Then
                ...
                lsvLista.Columns.Add("Fecha Emisión")
                lsvLista.Columns(2).Width = 120
                lsvLista.Columns.Add("Proveedor")
                lsvLista.Columns(3).Width = 250
                lsvLista.Columns.Add("Id Empresa")
                ...
                lsvLista.Columns.Add("Tipo Gasto")
    ...
    Private Sub frmImportarGastos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
